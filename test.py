...
def get_parser():
    ...
    parser.add_argument('--arch', type=str, default='FCA_FLP')
...
def get_model(args):
    ...
    if args.weight:
        weight_name = os.listdir(args.snapshot_path)[0]
        weight_path = osp.join(args.snapshot_path, weight_name)
...
def main():
    ...
    for val_id in range(val_num):
        ...
        FBIoU_array[val_id], mIoU_array[val_id], pIoU_array[val_id] = fb_iou, miou, piou
    ...
    print(
        'Best_mIoU: {:.4f} \t Best_FBIoU: {:.4f} \t Best_pIoU: {:.4f}'.format(
            mIoU_array.max(), FBIoU_array.max(), pIoU_array.max()))
    print(
        'Mean_mIoU: {:.4f} \t Mean_FBIoU: {:.4f} \t Mean_pIoU: {:.4f}'.format(
            mIoU_array[:-1].mean(), FBIoU_array[:-1].mean(), pIoU_array[:-1].mean()))
