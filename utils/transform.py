...
#from util import *
#from util import get_train_val_set, check_makdirs
import torch
...
class maskcrop( object):
    ...
    def __call__(self, image, label):
            ...
            if min(y1) < self.a:
                ...
                y_1 = min(y1)
            if max(x2) > width - self.a:
                x_2 = width - self.a
            else:
                x_2 = max(x2)
            if max(y2) > height - self.a:
                y_2 = height - self.a
            else:
                y_2 = max(y2)
            image = image[y_1 - self.a:y_2 + self.a, x_1 - self.a:x_2 + self.a]  # 分割图片
            label = label[y_1 - self.a:y_2 + self.a, x_1 - self.a:x_2 + self.a]  # 分割标签
            return image,label
