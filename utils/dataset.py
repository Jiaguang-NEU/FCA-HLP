...
class SemData(Dataset):
    def __init__(self, split=3, shot=1, data_root=None, base_data_root=None, data_list=None, data_set=None,
        ...
        print(self.base_data_root)
        self.base_path = os.getcwd() + os.path.join(self.base_data_root, mode, str(self.split)).split('.')[1]
        # 例如 '/root/data/ljg-w/BAM-main/data/base_annotation/pascal/train/0
        fss_list_root = './lists/{}/fss_list/{}/'.format(data_set, mode)  # eg. './lists/pascal/fss_list/train/'
        fss_data_list_path = fss_list_root + 'data_list_{}.txt'.format(split)  # eg. 'data_list_0.txt'
    ...
    def __getitem__(self, index):
        ...
        image_path = os.getcwd() + image_path.split('..')[1]
        label_path = os.getcwd() + label_path.split('..')[1]  # 加上根目录路径
        ...
        for k in range(self.shot):
            ...
            support_image_path = os.getcwd() + support_image_path.split('..')[1]  # 加上根目录
            support_label_path = os.getcwd() + support_label_path.split('..')[1]
        ...
        support_image_list = [[[] for _ in range(len(self.transform))] for _ in range(self.shot)]
        support_label_list = [[[] for _ in range(len(self.transform))] for _ in range(self.shot)]
        if self.transform is not None:
            image, label, label_b = self.transform_tri(image, label, label_b)  # transform the triple
            for k in range(self.shot):
                for j in range(len(self.transform)):
                    support_image_list[k][j], support_label_list[k][j] = self.transform[j](support_image_list_ori[k],
        ...
        for i in range(0, self.shot):
            s_x = s_xs[i][0].unsqueeze(0)
            for j in range(1, (len(self.transform))):
                s_x = torch.cat([s_xs[i][j].unsqueeze(0), s_x], 0)
            s_x_s.append(s_x)

        s_x1 = s_x_s[0].unsqueeze(0)
        for k in range(1, self.shot):
            s_x1 = torch.cat([s_x_s[k].unsqueeze(0), s_x1], 0)

        for i in range(0, self.shot):
            s_y = s_ys[i][0].unsqueeze(0)
            for j in range(1, (len(self.transform))):
                s_y = torch.cat([s_ys[i][j].unsqueeze(0), s_y], 0)
            s_y_s.append(s_y)
        s_y1 = s_y_s[0].unsqueeze(0)
        for k in range(1, self.shot):
            s_y1 = torch.cat([s_y_s[k].unsqueeze(0), s_y1], 0)

        # Return
        if self.mode == 'train':
            return image, label, label_b, s_x1, s_y1, subcls_list
        elif self.mode == 'val':
            return image, label, label_b, s_x1, s_y1, subcls_list, raw_label, raw_label_b
        elif self.mode == 'demo':
            total_image_list = support_image_list_ori.copy()
            total_image_list.append(raw_image)
            return image, label, label_b, s_x1, s_y1, subcls_list, total_image_list, support_label_list_ori, support_label_list_ori_mask, raw_label, raw_label_b
