...
cv2.setNumThreads(0)
def get_parser():  # obtain args
    parser = argparse.ArgumentParser(description='PyTorch Few-Shot Semantic Segmentation')
    parser.add_argument('--arch', type=str, default='FCA_HLP')
    parser.add_argument('--viz', action='store_true', default=False)
    parser.add_argument('--config', type=str, default='config/coco/coco_split2_resnet50.yaml',
                        help='config file')
    ...
    parser.add_argument('--exp_dir', type=str, default='error')
    parser.add_argument('--snapshot_dir', type=str, default='error')
    parser.add_argument('--result_dir', type=str, default='error')
    parser.add_argument('--show_dir', type=str, default='error')
...
def get_model(args):
    ...
    check_makedirs(args.snapshot_dir)
    check_makedirs(args.result_dir)

    if args.resume:
        resume_path = osp.join(args.snapshot_dir, args.resume)
...
def main():
    ...
    train_transform = transform.Compose([
        ...
        transform.RandomGaussianBlur(p=0.5),
        transform.RandomHorizontalFlip(p=0.5),
        transform.Crop([args.train_h, args.train_w], crop_type='rand', padding=mean, ignore_label=args.padding_label),
        transform.ToTensor(),
        transform.Normalize(mean=mean, std=std)])

    train_transform2 = transform.Compose([transform.maskcrop(),
                                          transform.Resize(args.train_h),
    ...
    if args.data_set == 'pascal' or args.data_set == 'coco':
        train_data = dataset.SemData(split=args.split, shot=args.shot, data_root=args.data_root,
                                     ...
                                     transform=[train_transform, train_transform2], transform_tri=train_transform_tri,
                                     mode='train', \
    ...
    if args.evaluate:
        if args.resized_val:
            ...
            val_transform2 = transform.Compose([transform.maskcrop(),
                                                transform.Resize(size=args.val_size),
                                                transform.ToTensor(),
                                                transform.Normalize(mean=mean, std=std)])
        ...
        if args.data_set == 'pascal' or args.data_set == 'coco':
            val_data = dataset.SemData(split=args.split, shot=args.shot, data_root=args.data_root,
                                       ...
                                       transform=[val_transform, val_transform2], transform_tri=val_transform_tri,
                                       mode='val', \
                                       data_set=args.data_set, use_split_coco=args.use_split_coco)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch % args.save_freq == 0) and (epoch > 0) and main_process():
            if osp.exists(resume_filename):
                os.remove(resume_filename)
            resume_filename = args.snapshot_dir + '/epoch_{}.pth'.format(epoch)
        ...
        if args.evaluate and epoch % 1 == 0:
            ...
            if mIoU > best_miou:
                ...
                if os.path.exists(filename):
                    os.remove(filename)
                if args.shot == 1:
                    filename = args.snapshot_dir + '/train_epoch_' + str(epoch) + '_{:.4f}'.format(best_miou) + '.pth'
                else:
                    filename = args.snapshot_dir + '/train{}_epoch_'.format(args.shot) + str(epoch) + '_{:.4f}'.format(
                        best_miou) + '.pth'
                if main_process():
    ...
    for i, (input, target, target_b, s_input, s_mask, subcls) in enumerate(train_loader):
        ...
        if args.evaluate and args.SubEpoch_val and (args.epochs <= 100 and epoch % 1 == 0 and epoch > 0) and (
            ...
            if mIoU > best_miou:
                ...
                keep_epoch = 0
                if args.shot == 1:
                    filename = args.snapshot_dir + '/train_epoch_' + str(epoch - 0.5) + '_{:.4f}'.format(
                        best_miou) + '.pth'
                else:
                    filename = args.snapshot_dir + '/train{}_epoch_'.format(args.shot) + str(
